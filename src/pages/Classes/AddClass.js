...
function AddClass() {
  ...
  const [desgniationIsValid, setDesignationIsValid] = useState(
  ...
  const handleDesgniation = (desgniation) => {
    if (
      desgniation.target.value !== undefined ||
      desgniation.target.value !== ""
    ) {
  ...
  const handleSubmit = async (classes) => {
    ...
    if (
      ...
      const response = await fetch(
        process.env.REACT_APP_API_URL + "/createClasse",
        {
          method: "POST",
          body: JSON.stringify({
            nom: name,
