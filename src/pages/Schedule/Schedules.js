...
function Schedules() {
  ...
  const handleConfirmAddEvent = async (e) => {
    ...
    sessionService.createSession(
      eventInformations.start.getHours(),
      eventInformations.start.getMinutes(),
      eventInformations.end.getHours() - eventInformations.start.getHours(),
      day,
      parseInt(emploiId.id),
      parseInt(eventInformations.teacherId),
      parseInt(currentUser.id),
      parseInt(eventInformations.matiereId),
      parseInt(eventInformations.roomId)
    );
  ...
  const handleEventClick = async (clickInfo) => {
    ...
    let eventById = await sessionService.getSessionById(
  ...
  const handleUpdateEvent = async () => {
    ...
    sessionService.updateSession(
      currentEvent.start_hour,
      currentEvent.start_minute,
      currentEvent.seance_duration,
      weekday.indexOf(currentEvent.day) + 1,
      currentEvent.emploiId,
      currentEvent.teacherId,
      currentEvent.adminId,
      currentEvent.matiereId,
      currentEvent.salleId,
      currentEvent.id
    );
    if (tooltipInstance) {
      tooltipInstance.dispose();
  ...
  const handleEventChange = (changeInfo) => {
    ...
    sessionService.updateSession(
      ...
      duration,
      start.getDay(),
      changeInfo.event._def.extendedProps.emploiId,
      changeInfo.event._def.extendedProps.teacherId,
      changeInfo.event._def.extendedProps.adminId,
      changeInfo.event._def.extendedProps.matiereId,
      changeInfo.event._def.extendedProps.roomId,
      parseInt(changeInfo.event._def.publicId)
    );
  };
