...
function EditCharge() {
  ...
  const [designationIsValid, setDesignationIsValid] = useState(
    "form-control is-valid"
  );
  const [codeFacture, setCodeFacture] = useState(charge.code_facture);
  const [codeFactureIsValid, setCodeFactureIsValid] = useState(
    "form-control is-valid"
  );
  const [montantFacture, setMtFacture] = useState(charge.montant_facture);
  const [montantFactureIsValid, setMtFactureIsValid] = useState(
    "form-control is-valid"
  );

  const handleDesignation = (designation) => {
    if (
      designation.target.value !== undefined &&
  ...
  const handleMtFacture = (montantFacture) => {
    if (
      montantFacture.target.value !== undefined &&
      montantFacture.target.value !== ""
    ) {
  ...
  const handleCodeFacture = (codeFacture) => {
    if (
      codeFacture.target.value !== undefined &&
      codeFacture.target.value !== ""
    ) {
  ...
  const handleSubmit = async (charge) => {
    ...
    if (
      designationIsValid === "form-control is-invalid" ||
      montantFactureIsValid === "form-control is-invalid" ||
      ...
      const response = await fetch(
        process.env.REACT_APP_API_URL + "/charge/" + id,
        {
          method: "PATCH",
          body: JSON.stringify({
                ...
                <Row>
                  ...
                  <Col xs={12} sm={6}>
                    <Form.Group>
                      ...
                      <Form.Control
                        className={montantFactureIsValid}
                        type="number"
                  ...
                  <Col xs={12} sm={12}>
                    <Form.Group>
                      ...
                      <Form.Control
                        className={codeFactureIsValid}
                        type="text"
                        defaultValue={codeFacture}
