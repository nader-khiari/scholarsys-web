...

    {
      ...
      cell: (row) => {
        ...
        classes.forEach((element) => {
          if (
            (typeof data.classeId === "number" &&
              data.classeId === element.id) ||
            (Array.isArray(data.classesId) &&
              data.classesId.length !== 0 &&
              data.classesId[0].split(",").includes(element.id.toString()))
          ) {
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <button
            className="btn btn-sm bg-success-light me-2"
            onClick={() => history.push("/edit-teacher/" + row.id)}
          >
